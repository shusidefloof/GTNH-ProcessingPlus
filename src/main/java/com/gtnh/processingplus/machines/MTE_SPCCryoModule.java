...
public class MTE_SPCCryoModule extends MTE_SPCModuleBase<MTE_SPCCryoModule> implements ISurvivalConstructable {
    ...
    private static IStructureDefinition<MTE_SPCCryoModule> STRUCTURE_DEFINITION = null;

    public MTE_SPCCryoModule(int aID, String aName, String aNameRegional) {
        super(aID, aName, aNameRegional);
    }

    public MTE_SPCCryoModule(String aName) {
        super(aName);
    }

    @Override
    public IMetaTileEntity newMetaEntity(IGregTechTileEntity aTileEntity) {
        return new MTE_SPCCryoModule(this.mName);
    }

    @Override
    public SPCModuleType getModuleType() {
        return SPCModuleType.CRYO;
    }

    @Override
    public IStructureDefinition<MTE_SPCCryoModule> getStructureDefinition() {
        if (STRUCTURE_DEFINITION == null) {
            STRUCTURE_DEFINITION = StructureDefinition.<MTE_SPCCryoModule>builder()
                .addShape(
                    STRUCTURE_PIECE_MAIN,
                    new String[][] { { "CCC", "C~C", "CCC" }, { "CCC", "CCC", "CCC" }, { "CCC", "CCC", "CCC" } })
                .addElement('C', ofBlock(GTNHPPBlocks.CASINGS, BlockGTNHPPCasings.CRYO_MODULE_CASING))
                .build();
        }
